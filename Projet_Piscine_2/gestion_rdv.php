...
if (isset($_GET['client_id']) && isset($_GET['propriete_adresse']) && isset($_GET['date']) && isset($_GET['heure']) && isset($_GET['status'])) {
    ...
    $sql = sprintf("INSERT INTO rendezvous (id, client_id, agent_id, propriete_id, date, heure, status) VALUES (%d, %d, %d, %s, %d, %s)", $conn->real_escape_string($new_id), $_SESSION['user_id'], $conn->real_escape_string($agent_id), $conn->real_escape_string($propriete_id), $conn->real_escape_string($date), $conn->real_escape_string($heure), 'confirmé');
}
